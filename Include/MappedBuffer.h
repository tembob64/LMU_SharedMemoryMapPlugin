...
public:
  ...
  bool Initialize(bool mapGlobally)
  {
    ...
    memset(mpMappedView, 0, sizeof(LMU_MappedBufferVersionBlock) + sizeof(BuffT));
...
private:
  ...
  HANDLE MapMemoryFile(char const* const fileName, bool dedicatedServerMapGlobally, LPVOID& pMappedView, LMU_MappedBufferVersionBlock*& pBufVersionBlock, BuffT*& pBuf) const
  {
    ...
    if (!isDedicatedServer  // Regular client use.
      ...
      hMap = ::CreateFileMappingA(
        ...
        PAGE_READWRITE,
        0  /*dwMaximumSizeLow*/,
        sizeof(LMU_MappedBufferVersionBlock) + sizeof(BuffT),
        mappingName);
    }
    else {
    ...
    // Set pointers up.
    pBufVersionBlock = static_cast<LMU_MappedBufferVersionBlock*>(mpMappedView);
    pBuf = reinterpret_cast<BuffT*>(static_cast<char*>(pMappedView) + sizeof(LMU_MappedBufferVersionBlock));
    assert((reinterpret_cast<char*>(pBufVersionBlock) + sizeof(LMU_MappedBufferVersionBlock)) == reinterpret_cast<char*>(pBuf));
  ...
  public:
    ...
    LMU_MappedBufferVersionBlock* mpWriteBuffVersionBlock = nullptr;
